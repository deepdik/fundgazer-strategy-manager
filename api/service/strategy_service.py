import json
import traceback
from api.models.strategy_model import (
    MasterPresetDataModel,
    FilteredStocksModel,
    UserPresetDataModel,
    PresetDataVersion,
)
from api.repository.strategy_repo import (
    save_user_preset_data,
    save_master_preset_data,
    save_filtered_stocks,
    get_master_preset_data,
    get_user_preset_data,
    get_filtered_stocks,
    save_stock_weightage,
)
...
from api.utils.strategy_logics.strategy.mpt_strategies import (
    Strategy_direct_comparision_index3,
)
...
async def first_time_run_strategy(
    symbols: list,
    timeframe: str,
    exchange: str,
    ms_id="0",
    ...
    try:
        ...
        for i in range(preset_count, 0, -1):
            print(f"{'-'*10}{i}{'-'*10}")
            data.preset_data(preset_count=i)
            print("Preset: ", data.get_latest_bar_datetime("NSE:GOLDBEES-EQ"))
            strategy.master_find_entry()
...
async def run_master_strategy(symbols: list, timeframe: str, exchange: str, ms_id: str):
    data = Live_DataHandler(symbols, timeframe, exchange)
    await data.init_data()
    print("Master date: ", data.get_latest_bar_datetime("NSE:GOLDBEES-EQ"))
    # data.preset_data(preset_count=preset_count)
...
async def run_user_strategy(
    ...
    preset_strategy_data = await get_user_preset_data(ms_id, user_id, version)
