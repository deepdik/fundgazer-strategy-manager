...
from api.repository.repo import save_user_preset_data, save_master_preset_data, save_filtered_stocks, \
...
async def run_master_strategy(symbols: list, timeframe: str, exchange: str,
    ...
    symbol_hash = create_symbol_hash(symbols)
    preset_data = await get_master_preset_data(symbol_hash, exchange,
                                               timeframe, index_cls.__name__)
    if preset_data:
        preset_data = json.loads(preset_data[0]["preset_data"])
...
async def run_user_strategy(symbols: list, timeframe: str,
    ...
    preset_strategy_data = await get_user_preset_data(symbol_hash, exchange, timeframe, index_cls.__name__)
    if preset_strategy_data:
        preset_strategy_data = json.loads(preset_strategy_data[0]["preset_data"])

    #preset_strategy_data, filter_stocks = await run_master_strategy(symbols, timeframe, exchange)
    filter_stocks = []
    filter_stocks_obj = await get_filtered_stocks(symbol_hash, exchange, timeframe, index_cls.__name__)
...
async def test_async():
    ...
    data = database.filtered_stocks.find({})
    for d in data:
        print("data-------", x)
    print("it is working")